...
const activeProducts = await productService.findAll({ // [!code focus]
...
const product = await productService.findById('prod-123') // [!code highlight]
...
const premiumProductsInStock = await productService.findBy({
  price: { gte: 1000 }, // [!code focus]
  inStock: true, // [!code focus]
  rating: { gte: 4.5 }, // [!code focus]
  categoryId: { in: ['electronics', 'computers'] } // [!code focus]
})
```
...
const engineeringManagers = await userService.findBy({
  role: 'manager', // [!code focus]
  department: 'engineering', // [!code focus]
  status: 'active' // [!code focus]
})
...
const inactiveUsers = await userService.findBy({
  lastLoginAt: { lt: threeMonthsAgo }, // [!code focus]
  status: 'active' // [!code focus]
})
