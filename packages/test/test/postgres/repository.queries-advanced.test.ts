...
describe('PG Service: Query Operations (With Options)', () => {
	...
	it('should find records using custom filtering logic with SQL expressions', async () => {
		...
		const todosWithSearch = await todosService.find({
			custom: and(
				...
				gt(sql`created_at`, Math.floor(cutoffDate.getTime() / 1000)), // Convert to Unix timestamp for PG
			),
		})
	...
	it('should find a single record with multiple relations', async () => {
		const todoWithUserTenant = await todosService.findOne(
			testIds.todoIds[0] ?? '',
			{
				relations: [
					{ type: 'left', table: users, sql: eq(todos.userId, users.id) },
		...
		expect(todoWithUserTenant?.tenantEntity.tenantId).toBe(
			todoWithUserTenant?.tenant,
		)
	})
